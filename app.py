...
if prompt := st.chat_input("Ask a physics question..."):
    ...
    if not gemini_key and not deepseek_key:
        st.error("⚠️ Please provide at least one API Key (Gemini or DeepSeek) in the sidebar.")
        st.stop()
    ...
    with st.spinner("Thinking..."):
        ...
        # 2. Fallback to DeepSeek if Gemini failed or wasn't provided
        if not response_text and deepseek_key:
            try:
                if gemini_key: # If we tried Gemini and failed, show a small toast
                    st.toast("Gemini unavailable, switching to DeepSeek...", icon="🔄")
