...
# ============================================================
# 5. LLM API CALLS (Gemini / OpenAI / DeepSeek)
# ============================================================
...
def call_gemini(messages, api_key, system_instruction):
    ...
    chat_history = []
    for m in messages[:-1]: # All except last (which is the new prompt)
...
def call_openai(messages, api_key, system_instruction):
...
with st.sidebar:
    ...
    if not openai_key:
        openai_key = st.text_input("ChatGPT (OpenAI) Key:", type="password")
    ...
    if st.button("🧹 Clear Chat"):
        st.session_state.messages = [{"role": "assistant", "content": "Hello! I'm your Feynman Physics tutor. What concept shall we explore today?"}]
        st.rerun()
...
if prompt := st.chat_input("Ask a physics question..."):
    ...
    if not gemini_key and not openai_key and not deepseek_key:
        st.error("⚠️ Please provide at least one API Key (Gemini, ChatGPT, or DeepSeek) in the sidebar.")
        st.stop()
    ...
    with st.spinner("Thinking..."):
        ...
        if gemini_key:
            try:
                ...
                # Fall through to next model if Gemini fails
                pass
        
        # 2. Try OpenAI (ChatGPT) if Gemini failed or key missing
        if not response_text and openai_key:
            try:
                if gemini_key: st.toast("Switching to ChatGPT...", icon="🔄")
                response_text = call_openai(st.session_state.messages, openai_key, current_instruction)
                used_model = "ChatGPT"
            except Exception as e:
                # Fall through to next model if OpenAI fails
                pass

        # 3. Try DeepSeek if both previous failed or keys missing
        if not response_text and deepseek_key:
            try:
                if gemini_key or openai_key: st.toast("Switching to DeepSeek...", icon="🔄")
    ...
    if response_text:
        ...
        st.error("❌ All API calls failed. Please check your keys or internet connection.")
