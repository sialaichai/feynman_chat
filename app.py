...
QUIZ_LEVELS = {
    ...
    "Advanced": "Advanced: More Complex problems involving 2 or more steps in calculations or linking multiple concepts together, or testing misconcpetions."
}
            ...
            with st.spinner(f"Generating {num_questions} {quiz_difficulty} quiz questions for {quiz_topic}..."):
                try:
                    ...
                    with st.expander("🐞 DEBUG: Raw Quiz Text (Before JSON Parsing)", expanded=True):
                        ...
                        st.code(response[:600], language="text")
                        st.caption(f"Total length: {len(response)} characters")
