...
# -----------------------------------------------------------------------------
# Store hashed password in .streamlit/secrets.toml
# Format: ACCESS_PASSWORD = "hashed_password_here"
# Generate hash using: hashlib.sha256("your_password".encode()).hexdigest()
...
def check_password():
    ...
    def password_entered():
        ...
        stored_hash = st.secrets.get("ACCESS_PASSWORD", "")
        
        # Check if hash matches
        if input_hash == stored_hash:
            st.session_state["password_correct"] = True
            # Store timestamp of successful login
    ...
    # Check auto-logout timeout (e.g., 8 hours = 28800 seconds)
    ...
    st.title("🔒 H2 Physics Feynman Bot - Login Required")
    ...
    with col2:
        ...
        st.markdown("Please enter the password to access the JPJC Physics tutor bot.")
        
        # Password input with type="password" to hide characters
        st.text_input(
            "Password:",
            type="password",
            on_change=password_entered,
            key="password",
            help="Enter the password provided by your instructor"
        )
        
        # Optional: Add a submit button as alternative
        if st.button("Login", type="primary", use_container_width=True):
            password_entered()
        
        st.markdown("---")
        st.caption("If you've forgotten the password, please contact your physics teacher.")
        
        # Don't show app content until logged in
        st.stop()
    
    return False

# -----------------------------------------------------------------------------
# 2. MAIN APP - ONLY SHOWS AFTER LOGIN
# -----------------------------------------------------------------------------
def main_app():
    """The main physics tutor application - only shown after login."""
  ...
  * **Instruction:** Move quickly through basic definitions. Focus on derivations, assumptions, and complex application. Challenge the student with extension questions. Skip basic algebraic steps.
  ...
  # Example: Just showing it works - replace with your actual code
    st.success("✅ Successfully logged in!")
    st.info("Your full physics tutor app would run here.")
